...
class Game:
  ...
  def set_players(self):
    ...
    print("************** 게임 접속 **************")
    for i in range(5):
      print("Player ", i + 1, "의 이름을 입력해 주세요: ", end="")
  ...
  def start_game(self):
    ...
    for i in range(5):
      print(self.player_list[i].name, "의 캐릭터 선택 차례입니다 1, 2, 3 중 하나의 값을 입력해 주세요 : ", end="")
      choice = int(input()) - 1
      self.player_list[i].speed = self.character_list[choice].speed
      print(self.player_list[i].name, "의 고유 속도는 시속 ", self.player_list[i].speed, "입니다")


    
    # TODO (3): 두 종류의 아이템을 생성해 아이템 목록(self.item_list)에 추가해주세요.
    self.item_list.append(Item("banana_slip", random.randint(-40, -20)))
    self.item_list.append(Item("booster", random.randint(30, 60)))
  ...
  def play_round(self):
    ...
    print("이번 라운드의 트랙 길이는", length, "km 입니다")
    ...
    for i in range(5):
      ...
      changed_speed = self.player_list[i].speed + item.speed_change

      self.player_list[i].round_speed = changed_speed
      print("Player", self.player_list[i].name, "는", item.item_type, "덕분에 시속", changed_speed, "로 이번 트랙을 질주합니다.")
    ...
    print("[라운드 결과]")
    for i in range(5):
      record = length/self.player_list[i].round_speed

      self.player_list[i].add_play_record(record)

      print("Player", self.player_list[i].name, "의 주행시간:", self.player_list[i].play_records[-1])
  ...
  def game_result(self):
    ...
    self.player_list.sort(key = lambda x: sum(x.play_records), reverse= True)

    for i in range(3):
      print(i+1, "위:", self.player_list[i].name, "(총 주행시간:", sum(self.player_list[i].play_records), "초)")
